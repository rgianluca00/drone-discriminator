...
#include "discriminator/Tracker.h"
using namespace std;
using namespace cv;
...
int main()
{
    ...
    while ( k != 1048689 ) //'q'
    {
        ...
        for ( const auto & x : m )
        {
            const int r = 5;
            // draw measurement
            cvCircle(img, Point(x(0), x(1)), 2*r, cv::Scalar(0, 0, 0));
            cvLine(img, Point(x(0) - r, x(1) - r), Point(x(0) + r, x(1) + r), cv::Scalar(0, 0, 0) );
            cvLine(img, Point(x(0) - r, x(1) + r),Point(x(0) + r, x(1) - r), cv::Scalar(0, 0, 0) );
        }
        ...
        for ( const auto & filter : tracker.filters() )
        {
            ...
            PutText(img, id.str(), Point2i(s(0) + 10, s(1)), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 0));
